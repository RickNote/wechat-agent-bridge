...
export function getSessionRecord(sessions, userId) {
  if (!sessions[userId]) {
    sessions[userId] = {
      threadId: null,
      lastMessageAt: null,
  ...
  if (!Array.isArray(session.approvedRules)) session.approvedRules = [];
  if (!("pendingApproval" in session)) session.pendingApproval = null;
  if (!("threadId" in session)) session.threadId = null;
  if (!("lastMessageAt" in session)) session.lastMessageAt = null;
  return session;
}
